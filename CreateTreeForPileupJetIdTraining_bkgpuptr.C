...
void CreateTreeForPileupJetIdTraining()
{
  ...
  TFile *outf   = TFile::Open("Background_Tr2016_forpuppi5Set.root","RECREATE");
  ...
  */
  ...
  //double progress = 10.0*ievp/(1.0*20000);
  //for(int ievp=40000;ievp<60000;ievp++) {                                                                        
  for(int ievp=80000;ievp<100000;ievp++) {
    double progress = 10.0*ievp/(1.0*80000);
    ...
    //for(int iev=40000;iev<60000;iev++) {
    for(int iev=80000;iev<100000;iev++) { 
	...
	for(unsigned j=0;j<pt_p->size();j++) {
	  ...
	  for(unsigned k=0;k<pt->size();k++) {
	    ...
	    if ((*pt)[k] > 50 && (*pt)[k] < 15 && (*genPt)[k] < 10) continue;

            if ((*genDR_p)[j] > 0.4 && (*flavorParton_p)[j] == 0 /*&& fabs((*eta)[j]) < 2.5 && */ && (*genDR)[k] > 0.4 && (*flavorParton)[k] == 0 /*&& fabs((*eta_p)[k]) < 2.5*/) {
  ...
  cout<<"0.0  - 2.5 bkg puppi: "<<trOUT2->GetEntries()<<endl;
  cout<<"2.5  - 2.75 bkg puppi: "<<trOUT4->GetEntries()<<endl;
  cout<<"2.75 - 3.0 bkg: puppi"<< trOUT6->GetEntries()<<endl;
  cout<<"3.0  - 5.0 bkg: puppi"<< trOUT8->GetEntries()<<endl; 
