...
void CreateTreeForPileupJetIdTraining()
{
  ...
  TFile *outf   = TFile::Open("Signal_Tr2016_forpuppi.root","RECREATE");
  ...
  */
    ...
    trOUT3->Branch("jetPt"             ,&pt_          ,"pt_/F");
    trOUT3->Branch("jetEta"            ,&eta_         ,"eta_/F");
    trOUT3->Branch("phi"               ,&phi_         ,"phi_/F");
    trOUT3->Branch("mass"              ,&mass_        ,"mass_/F");
    trOUT3->Branch("dR2Mean"           ,&dR2Mean_     ,"dR2Mean_/F");
    trOUT3->Branch("majW"              ,&majW_        ,"majW_/F");
    trOUT3->Branch("minW"              ,&minW_        ,"minW_/F");
    trOUT3->Branch("frac01"            ,&frac01_      ,"frac01_/F");
  ...
  */
    ...
    trOUT5->Branch("jetPt"             ,&pt_          ,"pt_/F");
    trOUT5->Branch("jetEta"            ,&eta_         ,"eta_/F");
    trOUT5->Branch("phi"               ,&phi_         ,"phi_/F");
    trOUT5->Branch("mass"              ,&mass_        ,"mass_/F");
    trOUT5->Branch("dR2Mean"           ,&dR2Mean_     ,"dR2Mean_/F");
    trOUT5->Branch("majW"              ,&majW_        ,"majW_/F");
    trOUT5->Branch("minW"              ,&minW_        ,"minW_/F");
    trOUT5->Branch("frac01"            ,&frac01_      ,"frac01_/F");
  ...
  */
    ...
    trOUT7->Branch("jetPt"             ,&pt_          ,"pt_/F");
    trOUT7->Branch("jetEta"            ,&eta_         ,"eta_/F");
    trOUT7->Branch("phi"               ,&phi_         ,"phi_/F");
    trOUT7->Branch("mass"              ,&mass_        ,"mass_/F");
    trOUT7->Branch("dR2Mean"           ,&dR2Mean_     ,"dR2Mean_/F");
    trOUT7->Branch("majW"              ,&majW_        ,"majW_/F");
    trOUT7->Branch("minW"              ,&minW_        ,"minW_/F");
    trOUT7->Branch("frac01"            ,&frac01_      ,"frac01_/F");
  ...
  for(int ievp=0;ievp<5000;ievp++) {
    double progress = 10.0*ievp/(1.0*5000);
    int k = TMath::FloorNint(progress); 
    if (k > decade) 
      cout<<10*k<<" %"<<endl;
    decade = k; 
    
    trIN_chs->GetEntry(ievp);
    //std::cout << " inside chs event number " << iev << std::endl;
    
    for(int iev=0;iev<5000;iev++) {
      trIN_puppi->GetEntry(iev);
	...
	for(unsigned j=0;j<pt_p->size();j++) {

	  if ((*pt_p)[j] > 50 && (*pt_p)[j] < 15 && (*genPt_p)[j] < 10) continue;
	  ...
	  for(unsigned k=0;k<pt_p->size();k++) {
	    ...
	    if ((*pt_p)[k] > 50 && (*pt_p)[k] < 15 && (*genPt_p)[k] < 10) continue;
            ...
            if ((*genDR)[j] < 0.01 && (*flavorParton)[j] != 0 /*&& fabs((*eta)[j]) < 2.5 && */ //&& (*genDR_p)[k] < 0.01 && (*flavorParton_p)[k] != 0 /*&& fabs((*eta_p)[k]) < 2.5*/) {
	  ...
	  genevtwt_ = genevtwt_p;
	  nVtx_         = nVtx_p;
	  npu_          = npu_p;
	  rho_          = rho_p;
	  flavorParton_ = (*flavorParton_p)[j];
	  nParticles_   = (*nParticles_p)[j];
	  nCharged_     = (*nCharged_p)[j];
	  pt_           = (*pt_p)[j];
	  eta_          = (*eta_p)[j];
	  genDR_        = (*genDR_p)[j];
	  genPt_        = (*genPt_p)[j];
	  beta_         = (*beta_p)[j];
	  betaStar_     = (*betaStar_p)[j];
	  pull_         = (*pull_p)[j];
	  jetR_         = (*jetR_p)[j];
	  jetRchg_      = (*jetRchg_p)[j];
	  dR2Mean_      = (*dR2Mean_p)[j];
	  majW_         = (*majW_p)[j];
	  minW_         = (*minW_p)[j];
	  frac01_       = (*frac01_p)[j];
	  frac02_       = (*frac02_p)[j];
  ...
  cout<<"0.0  - 2.5 signal puppi: "<<trOUT1->GetEntries()<<endl;
  cout<<"2.5  - 2.75 signal puppi: "<<trOUT3->GetEntries()<<endl;
  cout<<"2.75 - 3.0 signal: puppi"<< trOUT5->GetEntries()<<endl;
  cout<<"3.0  - 5.0 signal: puppi"<< trOUT7->GetEntries()<<endl; 
